...
- **The constellation reacts to the cursor**, learn-anything.xyz-style: each node is pushed away from the pointer within a radius and eases back via a spring once it passes (`useTransform` + `useSpring` per node, driven by two shared `mouseX`/`mouseY` motion values updated on `pointermove` — no React re-renders, so it stays smooth with ~40 nodes). One real bug worth flagging for future SVG work: the offset is applied via `style={{ translateX, translateY }}` on the inner `motion.g`, **not** `style={{ x, y }}` — Framer Motion's `x`/`y` map to native SVG position *attributes*, which only exist on shapes like `<circle>`/`<rect>` and are silently a no-op on `<g>`. `translateX`/`translateY` apply as a CSS transform instead, which works on any SVG element. It's also rendered full-bleed — `left-1/2 w-screen -translate-x-1/2` breaks it out of the page's normal `max-w-6xl` column to span the full viewport (capped at `1600px` so `aspect-square` doesn't get absurdly tall on ultrawide monitors); `body { overflow-x: clip }` in `globals.css` guards against the scrollbar-width sliver `100vw` can cause. Automatically inert under `prefers-reduced-motion` — the pointer position simply never updates, so every offset stays at rest.
