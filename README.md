...
| `content/timeline.ts` | Every career milestone (dates, titles, bullet points, chapter, placeholder flags, and the current-role `convergence` visual) |
...
## Current role: Manager, Commercial Analytics

The timeline, hero, résumé, and metadata all now present **Manager, Commercial Analytics — Commercial Operations, October 2026 – Present, Remote** as the current position (`content/timeline.ts`'s `commercial-analytics-manager` milestone, under a new "Commercial Strategy" chapter). It uses `isTurningPoint: true` for the dark-royal-red marker + pulse ring, and a `convergence` field (rendered by `components/timeline/MilestoneConvergence.tsx`) animating the listed input streams into "Commercial Analytics System" → Focus / Growth / Commercial Impact → "From Insight to Commercial Impact."

One assumption made along the way, flagged for confirmation: **the Asentech milestone's end date is set to October 2026** (the new role's start), closing what was previously an open-ended "Since July 20, 2023." If Asentech ended earlier, continued longer, or overlapped, update `content/timeline.ts`'s `asentech-analyst` milestone's `dateLabel`.

The old "What's Next" chapter/milestone (a generic "Currently: analytics, strategy...") has been removed entirely now that there's a real current role — the timeline instead continues subtly past the last milestone (a fading line and a soft pulsing dot, no card, no label) to suggest ongoing growth without inventing another chapter.
...
- **The timeline is the spine.** A GSAP ScrollTrigger–driven SVG path draws itself as the visitor scrolls (disabled/shown fully-drawn under `prefers-reduced-motion`; a straight-line `scaleY` trick doesn't work once the path curves, so it's drawn via `stroke-dasharray`/`stroke-dashoffset` instead). The line gently winds left and right rather than running straight, to pull the eye down the page — `lib/timeline-path.ts` builds a smooth S-curve through anchors, and every milestone marker's actual measured position (via a `ResizeObserver`, not a fixed guess) becomes one of those anchors, so a dot always sits exactly on the line even as it curves between them. Milestones alternate left/right on desktop and collapse to one column on mobile, and the "Moving to the United States" milestone is visually distinguished as a turning point (larger crimson marker with a pulse ring — the same treatment reused for the current-role milestone). Pre-U.S. cards use a dashed, tactile border; post-U.S. cards use a solid, structured border — a subtle nod to "exploratory/design-led" evolving into "structured/analytical."
...
- **The constellation reacts to the cursor** via a whole-graph parallax: every edge and node lives inside one shared `motion.g`, and that entire group tilts a few pixels toward the pointer (`useTransform` off two shared `mouseX`/`mouseY` motion values, smoothed with `useSpring`, capped at `PARALLAX_MAX`) rather than each node moving independently. An earlier version pushed each node away from the pointer individually within a radius; it looked lively but made nodes effectively unclickable, since the repulsion force is strongest at zero distance — the exact moment you try to click, the node flees hardest. Swapping to a shared-group parallax fixes this architecturally: nothing ever moves *relative to* what you're pointing at, so a click always lands. Each node still carries a generous invisible hit-target (`r=9`, versus a ~3-unit visible dot) since it now moves in lockstep with the visible dot rather than being a separate static circle. One real bug worth flagging for future SVG work: the offset is applied via `style={{ translateX, translateY }}` on the shared `motion.g`, **not** `style={{ x, y }}` — Framer Motion's `x`/`y` map to native SVG position *attributes*, which only exist on shapes like `<circle>`/`<rect>` and are silently a no-op on `<g>`. `translateX`/`translateY` apply as a CSS transform instead, which works on any SVG element. It's also rendered full-bleed — `left-1/2 w-screen -translate-x-1/2` breaks it out of the page's normal `max-w-6xl` column to span the full viewport (capped at `1600px` so `aspect-square` doesn't get absurdly tall on ultrawide monitors); `body { overflow-x: clip }` in `globals.css` guards against the scrollbar-width sliver `100vw` can cause. Automatically inert under `prefers-reduced-motion` — the pointer position simply never updates, so the graph stays at rest.
