...
- **The constellation reacts to the cursor**, learn-anything.xyz-style: each node is pushed away from the pointer within a radius and eases back via a spring once it passes (`useTransform` + `useSpring` per node, driven by two shared `mouseX`/`mouseY` motion values updated on `pointermove` — no React re-renders, so it stays smooth with ~40 nodes). It's also rendered noticeably larger than before (`max-w-5xl`, bigger circles and text) for readability. Automatically inert under `prefers-reduced-motion` — the pointer position simply never updates, so every offset stays at rest.
