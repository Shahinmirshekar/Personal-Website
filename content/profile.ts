export const profile = {
  name: "Shahin Mirshekari",
  currentTitle: "Data & Insights Analyst",
  heroKicker: "Design × Marketing × Analytics",
  heroSummary:
    "I find the story inside complexity. Design taught me how people see. Marketing taught me why people act. Statistics taught me how to test what is true. Today, I bring all three together to transform complex information into decisions, products, and experiences people can understand.",
  /** Literal résumé-style summary (from Shahin's résumé), used only on the
   * /resume print view — the hero above keeps its own editorial voice. */
  resumeSummary:
    "Insight-driven analytics professional with 6+ years of experience driving marketing performance through rigorous data analysis, forecasting, and experimentation. Adept at turning complex data into clear, actionable insights for marketing and sales teams. Skilled in SQL, Python, and visualization tools, with a strong record of automating workflows, aligning cross-functional goals, and delivering impact through marketing measurement and modeling.",
  manifesto: [
    "I don't see design, marketing, and analytics as separate disciplines.",
    "Design determines whether an idea can be understood.",
    "Marketing determines whether it matters to the audience.",
    "Analytics determines whether the evidence supports it.",
    "My work begins where those disciplines meet.",
  ],
  /** Rotating identity roles shown in the hero, resolving into heroKicker. */
  identityRoles: [
    "Data & Insights Analyst",
    "Marketing Analytics Strategist",
    "Visual Storyteller",
    "Dashboard Designer",
    "Community Builder",
    "Systems Thinker",
  ],
  closingHeadline: "The next chapter should be built around meaningful problems.",
  closingBody:
    "I'm most energized by work that requires analytical rigor, visual clarity, business judgment, and genuine curiosity about people.",
  /** External profile links. */
  sameAs: [
    "https://www.linkedin.com/in/shahinmirshekari/",
    "https://scholar.google.com/citations?user=VfSqvVkAAAAJ&hl=en",
  ],
} as const;
